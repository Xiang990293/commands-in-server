$give @p pale_oak_sign[block_entity_data={id:"minecraft:sign",front_text:{color:"black",has_glowing_text:0b,messages:[{click_event:{action:"run_command",command:"function mission_wall:interact_to_mission"},text:"[任務]"},"$(title)","",{"color":"black","selector":"@p"}]}},custom_data={"description":"$(description)","xp":"$(xp)","xp_level":"$(xp_level)","item":$(item),"reward_type":"$(reward_type)"},can_place_on=["minecraft:acacia_planks"]]
